                    ...
                    <div class="widget-content widget-content-area br-6">
                        <div class="row">
                            ...
                            <div class="col-xl-12 col-lg-12 col-sm-12 text-right">
                                <button class="btn btn-info view_documents" data-toggle="modal" data-target="#exampleModal" id="<?= $feuser['document'] ?>">Show Documents</button>
                                <?php
                                    if($feuser['status'] == 0){
                                ?>
                                        <button class="btn btn-success changeStatus" key="1" id="<?= $feuser['id'] ?>">Activate</button>
                                <?php
                                    } else {
                                ?>
                                        <button class="btn btn-warning changeStatus" key="0" id="<?= $feuser['id'] ?>">Deactivate</button>
                                <?php
                                    }
                                ?>
                                <?php
                                    if($feuser['status'] == 0){
                                ?>
                                <button class="btn btn-danger disapproved" id="<?= $feuser['id'] ?>">Disapproved</button>
                                <?php
                                    }
                                ?>
                            </div>
                    ...
                    <div class="widget-content widget-content-area br-6">
                        <div class="row">
                            ...
                            <div class="col-md-2 scbuttonpadding">
                                <button type="button" class="form-control btn btn-success" id="ordercheck" name="ordercheck">to save</button>
                            </div>
                            <div class="col-md-3 scbuttonpadding">
                                <button type="button" class="form-control btn btn-info" id="resetdefault" name="resetdefault">Reset to default</button>
                            </div>
                        </div>
                        <hr>
    ...
    $( "#ordercheck" ).click(function() {
        ...
        var dataTable1 = $('#display_order').DataTable({
            ...
            "ajax": {
                url: "code/display_delivery_order.php",
                type: "POST",
                data: {user_id:user_id,status:1,start_date:start_date,end_date:end_date},
            }
        });
    });
...
<script type="text/javascript">
    $(document).on('click', '.view_documents', function() {
        ...
        $('#plan_details').html('<img src="../assets/img/user/'+image+'" style = "border-radius: 50px;width: 100%" height="350" />');
    });
</script>
<script type="text/javascript">
    $(document).ready(function() {
    ...
    function SwalStatusChange(user_id,status) {
        Swal.fire({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    $.ajax({
                        url: 'code/delivery_boy_status_change.php',
                        type: 'POST',
...
<script type="text/javascript">
        ...
        var bindValidator = function () {
            ...
            var validateFields = {
                startDate: {
                    validators: {
                        notEmpty: { message: 'This field has to be filled in.' },
                        callback: {
