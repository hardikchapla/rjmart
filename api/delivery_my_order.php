<?php
	...
	if(!empty($_REQUEST)){
 		...
 		if($checkmobile->rowCount() > 0){
			...
			$order_details = $db->query("SELECT c.id as request_id,c.*, a.id as order_id,a.created as orderdt,a.*,b.* FROM near_by_request c LEFT JOIN product_order a ON c.order_id = a.id LEFT JOIN user_address b ON c.from_id = b.user_id WHERE c.to_id = '$user_id' AND (a.order_status = 1 OR a.order_status = 4) AND c.status = 1  ORDER BY c.created DESC");
			$aa = array();
			$a = 0;
